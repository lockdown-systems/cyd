...
const filteredStories = computed(() => {
  ...
  return archiveData.value.stories.filter(story => {
    ...
    return (story.text && story.text.toLocaleLowerCase().includes(s)) ||
        (story.title && story.title.toLocaleLowerCase().includes(s)) ||
        (story.lifeEventTitle && story.lifeEventTitle.toLocaleLowerCase().includes(s)) ||
        (story.user.name.toLocaleLowerCase().includes(s)) ||
        (story.attachedStory && story.attachedStory.text && story.attachedStory.text.toLocaleLowerCase().includes(s));
  });        
});
</script>
...
<template>
  <div class="posts-container">
    <div class="filter-container">
      ...
      <p class="text-center text-muted small">Showing {{ filteredStories.length.toLocaleString() }} posts
      </p>
    </div>
