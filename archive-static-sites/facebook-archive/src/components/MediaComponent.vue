...
<template>
    <div class="media-item p-1 border rounded text-center">
        <template v-if="media.mediaType == 'Photo' || media.mediaType == 'GenericAttachmentMedia'">
            <!-- image -->
             <a v-if="media.filename" :href="filename" target="_blank">
                <img :src="filename" :alt="media.accessibilityCaption" :title="media.accessibilityCaption">
            </a>
            <p v-else class="text-muted small">ERROR: No filename specified</p>
        </template>
        <template v-else-if="media.mediaType == 'Video'">
            <!-- video -->
            <p v-if="media.needsVideoDownload" class="text-muted small">TODO: video is not supported yet</p>
            <div v-else>
        ...
        <template v-else>
            <!-- other media types -->
            <p class="text-muted small">ERROR: unknown media type: {{ media.mediaType }}</p>
        </template>
        <p v-if="media.title" class="text-muted small mt-1" v-html="formattedTitle"></p>
    </div>
</template>
