...
const openCydURL = async (cydURL: string) => {
  ...
  if (url.hostname == "open") {
    ...
    if (win) {
      log.info(
        "Sending Cyd Open callback event to renderer:",
        cydOpenEventName,
      );
...
async function initializeApp() {
  ...
  try {
    ...
    dialog.showErrorBox(
      "Cyd Error",
      "Failed to run database migrations. The application will now exit.",
    );
    app.quit();
    return;
  }
...
async function createWindow() {
  ...
  const icon = nativeImage.createFromPath(
    path.join(getResourcesPath(), "icon.png"),
  );
  ...
  if (!global.ipcHandlersRegistered) {
    ...
    ipcMain.handle(
      "showQuestion",
      async (_, message: string, trueText: string, falseText: string) => {
        try {
    ...
    ipcMain.handle(
      "loadFileInWebview",
      async (_, webContentsId: number, filename: string) => {
        try {
    ...
    ipcMain.handle("startPowerSaveBlocker", async (_): Promise<number> => {
      const powerSaveBlockerID = powerSaveBlocker.start(
    ...
    ipcMain.handle(
      ...
      async (_, powerSaveBlockerID: number) => {
