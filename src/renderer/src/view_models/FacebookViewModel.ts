...
export function findCurrentUserInitialData(
  data: unknown,
...
export class FacebookViewModel extends BaseViewModel {
  ...
  async refreshDatabaseStats() {
    this.databaseStats = await window.electron.Facebook.getDatabaseStats(
      this.account.id,
    );
  ...
  async syncProgress() {
    await window.electron.Facebook.syncProgress(
      this.account?.id,
    ...
    while (true) {
      ...
      try {
        ...
        if (e instanceof InternetDownError) {
          ...
          await this.error(
            AutomationErrorType.facebook_loadURLError,
            {
              url: url,
              error: formatError(e as Error),
            },
  ...
  async login() {
    ...
    while (true) {
      const c_user = await window.electron.Facebook.getCookie(
        this.account?.id,
        "c_user",
      );
      if (c_user !== null) {
    ...
    if (profilePictureURI && this.account && this.account.facebookAccount) {
      this.account.facebookAccount.profileImageDataURI =
        await window.electron.Facebook.getProfileImageDataURI(
          this.account.id,
          profilePictureURI,
        );
    }
    ...
    if (
      url.startsWith("https://www.facebook.com/two_factor/remember_browser/")
    ) {
  ...
  async runJobSavePosts(jobIndex: number): Promise<boolean> {
    await window.electron.trackEvent(
      PlausibleEvents.FACEBOOK_JOB_STARTED_SAVE_POSTS,
      navigator.userAgent,
    );
    ...
    const managedPostsButtonSelector =
      'div[role="main"] > div:last-child > div:last-child > div > div:last-child > div:nth-child(2) > div > div > div:first-child > div:last-child > div > div:last-child > div[role="button"]';
    try {
      // wait for the "Manage posts" button to appear
      await this.waitForSelector(
        managedPostsButtonSelector,
        facebookProfileURL,
      );
  ...
  async runJobSavePostsHTML(jobIndex: number): Promise<boolean> {
    await window.electron.trackEvent(
      PlausibleEvents.FACEBOOK_JOB_STARTED_SAVE_POSTS_HTML,
      navigator.userAgent,
    );
  ...
  async runJobDeletePosts(jobIndex: number) {
    await window.electron.trackEvent(
      PlausibleEvents.FACEBOOK_JOB_STARTED_DELETE_POSTS,
      navigator.userAgent,
    );
    ...
    try {
      postsToDelete = await window.electron.Facebook.deletePostsStart(
        this.account.id,
      );
    } catch (e) {
      await this.error(
        AutomationErrorType.facebook_runJob_deletePosts_FailedToStart,
        {
          error: formatError(e as Error),
        },
      );
    ...
    this.log(
      "runJobDeletePosts",
      `found ${postsToDelete.posts.length} posts to delete`,
    );
    ...
    for (let i = 0; i < postsToDelete.posts.length; i++) {
      ...
      for (let tries = 0; tries < 3; tries++) {
        ...
        if (statusCode == 200) {
          ...
          try {
            await window.electron.Facebook.deletePost(
              this.account.id,
              currentPostItem.id,
              "post",
            );
            ...
            await this.error(
              AutomationErrorType.facebook_runJob_deletePosts_FailedToUpdateDeleteTimestamp,
              {
                error: formatError(e as Error),
              },
              {
          ...
          this.log("runJobDeleteposts", [
            "statusCode",
            statusCode,
            "failed to delete post, try #",
            tries,
          ]);
          await this.sleep(1000);
        }
      ...
      if (!postDeleted) {
        await this.error(
          AutomationErrorType.facebook_runJob_deletePosts_FailedToDelete,
          {
  ...
  async runJobArchiveBuild(jobIndex: number): Promise<boolean> {
    await window.electron.trackEvent(
      PlausibleEvents.FACEBOOK_JOB_STARTED_ARCHIVE_BUILD,
      navigator.userAgent,
    );
...
**To get started, log in to your Facebook account below.**`;
        ...
        case State.WizardStart:
          ...
          if (
            !this.account.facebookAccount?.accountID ||
            this.account.facebookAccount?.accountID === "0"
          ) {
            this.state = State.Login;
            break;
          }
...
**Which data do you want to delete?**`;
        ...
        case State.RunJobs:
          this.progress = await window.electron.Facebook.resetProgress(
            this.account.id,
          );
