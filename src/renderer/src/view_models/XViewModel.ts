...
export class XViewModel extends BaseViewModel {
  ...
  async indexTweetsHandleRateLimit(): Promise<boolean> {
    ...
    if (await this.doesSelectorExist('section [data-testid="cellInnerDiv"]')) {
      ...
      let numberOfDivsBefore = await this.countSelectorsFound(
      ...
      await this.sleep(2000);
      let numberOfDivsAfter = await this.countSelectorsFound(
        "section div[data-testid=cellInnerDiv]",
      );
      ...
      if (
        (await this.countSelectorsWithinElementLastFound(
      ...
      numberOfDivsBefore = await this.countSelectorsFound(
        "section div[data-testid=cellInnerDiv]",
      );
      ...
      numberOfDivsAfter = await this.countSelectorsFound(
        "section div[data-testid=cellInnerDiv]",
      );
      ...
      // Count divs after clicking retry button
      const numberOfDivsAfter = await this.countSelectorsFound(
        "section div[data-testid=cellInnerDiv]",
      );
  ...
  async indexTweetsVerifyThereIsNoMore(): Promise<boolean> {
    this.log(
      ...
      "verifying there is no more tweets",
    );
    await this.scrollToBottom();
    ...
    this.log("indexTweetsVerifyThereIsNoMore", [
      "parsed tweets",
      this.progress,
    ]);
...
Hang on while I scroll down to your earliest tweets.`;
    ...
    while (this.progress.isIndexTweetsFinished === false) {
      ...
      // Parse so far
      try {
        this.progress = await window.electron.X.indexParseTweets(
          this.account.id,
        );
        this.log("runJobIndexTweets", ["parsed tweets", this.progress]);
      } catch (e) {
        const latestResponseData =
          await window.electron.X.getLatestResponseData(this.account.id);
        await this.error(
...
Hang on while I scroll down to your earliest direct message conversations...`;
    ...
    while (this.progress.isIndexConversationsFinished === false) {
      ...
      // Parse so far
      try {
        this.progress = await window.electron.X.indexParseConversations(
          this.account.id,
        );
      } catch (e) {
        const latestResponseData =
...
Please wait while I index all the messages from each conversation...`;
    ...
    this.progress.conversationMessagesIndexed =
      this.progress.totalConversations -
    ...
    for (
      let i = 0;
      i < indexMessagesStartResponse.conversationIDs.length;
      i++
    ) {
      await this.waitForPause();
      ...
      while (this.progress.isIndexMessagesFinished === false) {
        ...
        // Scroll to top
        await window.electron.X.resetRateLimitInfo(this.account.id);
        let moreToScroll = await this.scrollToTop(
        ...
        // Parse so far
        try {
          this.progress = await window.electron.X.indexParseMessages(
            this.account.id,
          );
        } catch (e) {
          const latestResponseData =
...
Hang on while I scroll down to your earliest bookmarks.`;
  ...
  async runJobDeleteTweets(jobIndex: number) {
    ...
    try {
      tweetsToDelete = await window.electron.X.deleteTweetsStart(
        this.account.id,
      );
    } catch (e) {
      await this.error(
  ...
  async runJobDeleteRetweets(jobIndex: number) {
    ...
    try {
      tweetsToDelete = await window.electron.X.deleteRetweetsStart(
        this.account.id,
      );
    } catch (e) {
      await this.error(
    ...
    this.showAutomationNotice = true;
    await this.loadURLWithRateLimit(
      "https://x.com/" + this.account.xAccount?.username,
    );
  ...
  async runJobDeleteLikes(jobIndex: number) {
    ...
    try {
      tweetsToDelete = await window.electron.X.deleteLikesStart(
        this.account.id,
      );
    } catch (e) {
  ...
  async runJobDeleteDMs(jobIndex: number): Promise<boolean> {
    ...
    while (true) {
      ...
      for (tries = 0; tries < 3; tries++) {
        ...
        if (
          !(await this.scriptMouseoverElementFirst(
        ...
        if (
          !(await this.scriptClickElementWithinElementFirst(
            'div[data-testid="conversation"]',
            "button",
          ))
        ) {
    ...
    while (true) {
      ...
      for (tries = 0; tries < 3; tries++) {
        ...
        if (reloadFollowingPage) {
          ...
          numberOfAccountsToUnfollow = await this.countSelectorsFound(
            'div[data-testid="cellInnerDiv"] button button',
          );
        ...
        if (this.progress.isUnfollowEveryoneFinished) {
          this.log("runJobUnfollowEveryone", [
        ...
        if (
          !(await this.scriptMouseoverElementNth(
            'div[data-testid="cellInnerDiv"] button button',
        ...
        if (
          !(await this.scriptClickElementNth(
            'div[data-testid="cellInnerDiv"] button button',
            accountToUnfollowIndex,
          ))
        ) {
        ...
        if (!errorTriggered) {
          ...
          await window.electron.X.setConfig(
            ...
            `${this.progress.accountsUnfollowed}`,
          );
  ...
  async runJobMigrateBluesky(jobIndex: number): Promise<boolean> {
    ...
    const tweetCounts: XMigrateTweetCounts =
      await window.electron.X.blueskyGetTweetCounts(this.account.id);
  ...
  async runJobMigrateBlueskyDelete(jobIndex: number): Promise<boolean> {
    ...
    // Start the progress
    await this.syncProgress();
    this.progress.totalMigratedPostsToDelete =
