...
const buttonClicked = async () => {
    console.log('FacebookLastImportOrBuildComponent', 'buttonClicked', props.buttonText, props.buttonState);
    emit('setState', props.buttonState);
};

onMounted(async () => {
    const lastFinishedJob_importArchive = await window.electron.Facebook.getConfig(props.accountID, 'lastFinishedJob_importArchive');
    if (lastFinishedJob_importArchive) {
        lastImportArchive.value = new Date(lastFinishedJob_importArchive);
    }

    const lastFinishedJob_indexTweets = await window.electron.Facebook.getConfig(props.accountID, 'lastFinishedJob_indexTweets');
    const lastFinishedJob_indexLikes = await window.electron.Facebook.getConfig(props.accountID, 'lastFinishedJob_indexLikes');
...
<template>
    <div class="alert alert-light small text-center" role="alert">
        <div v-if="lastImportArchive">
            You last imported your Facebook archive {{ formatDistanceToNow(lastImportArchive, {
                addSuffix: true
            }) }}.
        </div>
        ...
        <div v-if="!lastImportArchive && !lastBuildDatabase">
            <div>
                Cyd can unfriend everyone right away
            </div>
            <div>
                <i class="fa-solid fa-triangle-exclamation" />
                You'll need to import or build your database to delete your Facebook posts
            </div>
        </div>
