...
const saveSettings = async () => {
    ...
    if (!props.model.account) {
        console.error('FacebookWizardBuildOptionsPage', 'saveSettings', 'account is null');
        return;
    }
    const account = await window.electron.database.getAccount(props.model.account?.id);
    if (account && account.facebookAccount) {
        ...
        <form @submit.prevent>
            <div class="mb-3">
                <div class="form-check">
                    <input id="savePosts" v-model="savePosts" type="checkbox" class="form-check-input">
            ...
            <div class="indent">
                <div class="mb-3">
                    <div class="form-check">
                        <input id="savePostsHTML" v-model="savePostsHTML" type="checkbox" class="form-check-input"
                            :disabled="!savePosts">
                    ...
                    <div class="indent">
                        <small class="form-text text-muted">
                            Make an HTML archive of each post, including its comments, which is good
