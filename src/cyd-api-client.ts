...
export default class CydAPIClient {
  ...
  async postXProgress(
    request: PostXProgressAPIRequest,
    ...
    try {
      ...
      if (response.status != 200) {
        return this.returnError(
          "Failed to post XProgress with the server.",
          response.status,
        );
      }
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      if (error.response) {
        ...
        return this.returnError(
          `Failed to post XProgress with the server. Status code: ${statusCode}, Response body: ${JSON.stringify(responseBody)}`,
        );
      } else if (error.request) {
        return this.returnError(
          "Failed to post XProgress with the server. No response received.",
        );
      } else {
        return this.returnError(
  ...
  async postAutomationErrorReport(
    request: PostAutomationErrorReportAPIRequest,
    authenticated: boolean,
  ): Promise<boolean | APIErrorResponse> {
    ...
    try {
      ...
      if (error.response) {
        ...
        const responseBody = error.response.data;
        return this.returnError(
          `Failed to post automation error report with the server. Status code: ${statusCode}, Response body: ${JSON.stringify(responseBody)}`,
        );
      } else if (error.request) {
        return this.returnError(
          "Failed to post automation error report with the server. No response received.",
        );
      } else {
        return this.returnError(
          `Failed to post automation error report with the server. Error: ${error.message}`,
        );
      }
    }
  ...
  async getVersion(): Promise<GetVersionAPIResponse | APIErrorResponse> {
    ...
    try {
      ...
      return this.returnError(
        "Failed to get current version. Maybe the server is down?",
      );
    }
  }
}
