...
const storySQL =
  "SELECT media.*, media_story.storyID AS storyID FROM media LEFT JOIN media_story ON media.mediaID = media_story.mediaID WHERE media_story.storyID = ?";
const attachedStorySQL =
  "SELECT media.*, media_attached_story.storyID AS storyID FROM media LEFT JOIN media_attached_story ON media.mediaID = media_attached_story.mediaID WHERE media_attached_story.storyID = ?";
...
test("FacebookAccountController.parseAPIResponse() for http1", async () => {
  ...
  const userRows: FacebookUserRow[] = database.exec(
    controller.db,
    "SELECT * FROM user",
    [],
    "all",
  ...
  const storyRows: FacebookStoryRow[] = database.exec(
    controller.db,
    "SELECT * FROM story",
    [],
    "all",
  ...
  expect(storyRows[0].storyID).toBe(
    "UzpfSTYxNTcyNzk4MjI3MDE4OjEyMjEyODIyOTYxODc1OTk0MDoxMjIxMjgyMjk2MTg3NTk5NDA=",
  );
  expect(storyRows[0].attachedStoryID).toBe(
    "UzpfSTEwMDA0NDE3Nzg3MjM0MzoxMjI0NTgyMzQ5MDI0MzQ5OjEyMjQ1ODIzNDkwMjQzNDk=",
  );
  expect(storyRows[1].storyID).toBe(
    "UzpfSTYxNTcyNzk4MjI3MDE4OjEyMjExMzg5NTU1ODc1OTk0MDoxMjIxMTM4OTU1NTg3NTk5NDA=",
  );
  expect(storyRows[2].storyID).toBe(
    "UzpfSTYxNTcyNzk4MjI3MDE4OjEyMjExMzg5NTE3NDc1OTk0MDoxMjIxMTM4OTUxNzQ3NTk5NDA=",
  );
  expect(storyRows[3].storyID).toBe(
    "UzpfSTYxNTcyNzk4MjI3MDE4OjEyMjExMzg5NDgzMjc1OTk0MDoxMjIxMTM4OTQ4MzI3NTk5NDA=",
  );
  expect(storyRows[3].attachedStoryID).toBe(
    "UzpfSTEwMDAxNTY0MDgzOTU5MjpWSzoxNjgyMTUwNTc2MDM0Nzkw",
  );
  expect(storyRows[4].storyID).toBe(
    "UzpfSTYxNTcyNzk4MjI3MDE4OjEyMjExMjgzNjY3Mjc1OTk0MDoxMjIxMTI4MzY2NzI3NTk5NDA=",
  );
  expect(storyRows[5].storyID).toBe(
    "UzpfSTYxNTcyNzk4MjI3MDE4OjEyMjExMjgzNjQzMjc1OTk0MDoxMjIxMTI4MzY0MzI3NTk5NDA=",
  );
  expect(storyRows[6].storyID).toBe(
    "UzpfSTYxNTcyNzk4MjI3MDE4OjEyMjEwNjMyOTM3Mjc1OTk0MDoxMjIxMDYzMjkzNzI3NTk5NDA=",
  );
  expect(storyRows[7].storyID).toBe(
    "UzpfSTYxNTcyNzk4MjI3MDE4OjEyMjEwNjMxOTk2NDc1OTk0MDoxMjIxMDYzMTk5NjQ3NTk5NDA=",
  );
  expect(storyRows[8].storyID).toBe(
    "UzpfSTYxNTcyNzk4MjI3MDE4OjEyMjEwNjMxOTgyNjc1OTk0MDoxMjIxMDYzMTk4MjY3NTk5NDA=",
  );

  // there should be 2 attached stories
  const attachedStoryRows: FacebookAttachedStoryRow[] = database.exec(
    controller.db,
    "SELECT * FROM attached_story",
    [],
    "all",
  ) as FacebookAttachedStoryRow[];
  expect(attachedStoryRows.length).toBe(2);
  expect(attachedStoryRows[0].storyID).toBe(
    "UzpfSTEwMDA0NDE3Nzg3MjM0MzoxMjI0NTgyMzQ5MDI0MzQ5OjEyMjQ1ODIzNDkwMjQzNDk=",
  );
  expect(attachedStoryRows[1].storyID).toBe(
    "UzpfSTEwMDAxNTY0MDgzOTU5MjpWSzoxNjgyMTUwNTc2MDM0Nzkw",
  );
  ...
  let storyID =
    "UzpfSTYxNTcyNzk4MjI3MDE4OjEyMjExMzg5NTU1ODc1OTk0MDoxMjIxMTM4OTU1NTg3NTk5NDA=";
  let mediaRows: ExtendedFacebookMediaRow[] = database.exec(
    controller.db,
    storySQL,
    [storyID],
    "all",
  ) as ExtendedFacebookMediaRow[];
...
test("FacebookAccountController.parseAPIResponse() for http2", async () => {
  ...
  expect(storyRows[0].storyID).toBe(
    "UzpfSTYxNTcyNzk4MjI3MDE4OjEyMjEwNjMxOTIwODc1OTk0MDoxMjIxMDYzMTkyMDg3NTk5NDA=",
  );
  expect(storyRows[1].storyID).toBe(
    "UzpfSTYxNTcyNzk4MjI3MDE4OjE2NjA4NzYwOTE0NzMxMTA6MTY2MDg3NjA5MTQ3MzExMA==",
  );
  expect(storyRows[1].title).toBe("Chase Westbrook updated his cover photo.");
  expect(storyRows[2].storyID).toBe(
    "UzpfSTYxNTcyNzk4MjI3MDE4OjEyMjEwNjMxODg5MDc1OTk0MDo0NTI2NjQ1NTExNDU0MzY=",
  );
  expect(storyRows[2].title).toBe(
    "Chase Westbrook updated his profile picture.",
  );
  expect(storyRows[3].storyID).toBe(
    "UzpfSTYxNTcyNzk4MjI3MDE4OjEyMjEwNjMxNzI0MDc1OTk0MDoxMjIxMDYzMTcyNDA3NTk5NDA=",
  );
  ...
  expect(mediaRows.length).toBe(1);
  expect(mediaRows[0].mediaID).toBe("122106319022759940");
  expect(mediaRows[0].accessibilityCaption).toBe(
