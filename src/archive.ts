...
const getArchivePath = (account: Account): string | null => {
  if (account.type == "X" && account.xAccount && account.xAccount.username) {
    ...
    account.facebookAccount.accountID &&
    ...
    return getAccountDataPath(
      "Facebook",
      `${account.facebookAccount.accountID} ${account.facebookAccount.name}`,
    );
  }
  return null;
};
...
export const defineIPCArchive = () => {
  ipcMain.handle(
    ...
    async (_, outputPath: string, basename: string): Promise<boolean> => {
      try {
  ...
  ipcMain.handle(
    "archive:openFolder",
    async (_, accountID: number, folderName: string): Promise<void> => {
  ...
  ipcMain.handle(
    "archive:getInfo",
    async (_, accountID: number): Promise<ArchiveInfo> => {
      ...
      archiveInfo.folderEmpty =
        !fs.existsSync(archivePath) || fs.readdirSync(archivePath).length === 0;
