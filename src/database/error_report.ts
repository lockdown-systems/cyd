...
export const getErrorReport = (id: number): ErrorReport | null => {
  const row: ErrorReportRow | undefined = exec(
    getMainDatabase(),
    "SELECT * FROM errorReport WHERE id = ?",
    [id],
    "get",
  ) as ErrorReportRow | undefined;
  if (!row) {
    return null;
  }
  return {
...
export const getNewErrorReports = (accountID: number): ErrorReport[] => {
  const rows: ErrorReportRow[] = exec(
    getMainDatabase(),
    "SELECT * FROM errorReport WHERE accountID = ? AND status = ?",
    [accountID, "new"],
    "all",
  ) as ErrorReportRow[];
...
export const dismissNewErrorReports = (accountID: number) => {
  exec(
    getMainDatabase(),
    "DELETE FROM errorReport WHERE accountID = ? AND status = ?",
    [accountID, "new"],
  );
};
...
export const defineIPCDatabaseErrorReport = () => {
  ipcMain.handle(
    "database:getErrorReport",
    async (_, id): Promise<ErrorReport | null> => {
      try {
  ...
  ipcMain.handle(
    "database:getNewErrorReports",
    async (_, accountID: number): Promise<ErrorReport[]> => {
      try {
  ...
  ipcMain.handle(
    "database:updateErrorReportSubmitted",
