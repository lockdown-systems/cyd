...
function facebookAccountRowToFacebookAccount(row: FacebookAccountRow): FacebookAccount {
    return {
        id: row.id,
...
export const getFacebookAccount = (id: number): FacebookAccount | null => {
    const row: FacebookAccountRow | undefined = exec(getMainDatabase(), 'SELECT * FROM facebookAccount WHERE id = ?', [id], 'get') as FacebookAccountRow | undefined;
    if (!row) {
        return null;
    }
...
export const getFacebookAccounts = (): FacebookAccount[] => {
    const rows: FacebookAccountRow[] = exec(getMainDatabase(), 'SELECT * FROM facebookAccount', [], 'all') as FacebookAccountRow[];
...
export const createFacebookAccount = (): FacebookAccount => {
    const info: Sqlite3Info = exec(getMainDatabase(), 'INSERT INTO facebookAccount DEFAULT VALUES') as Sqlite3Info;
