...
export class MITMController implements IMITMController {
  ...
  async startMITM(
    ...
    log.info(
      `MITMController: Account ${this.account?.id}, starting proxy`,
      proxyFilter,
    );
            ...
            ctx.onResponseEnd(async (ctx, callback) => {
              ...
              try {
                responseData.responseBody = (
                  await HttpEncoding.decodeBuffer(
                    buffer,
    ...
    if (fs.existsSync(certsPath)) {
      log.debug(
        `MITMController: Account ${this.account?.id}, Deleting old certificates dir: ${certsPath}`,
      );
      fs.rmSync(certsPath, { recursive: true });
    } else {
      log.debug(
        `MITMController: Account ${this.account?.id}, Certificates dir: ${certsPath}`,
      );
    }
    ...
    ses.setCertificateVerifyProc((request, callback) => {
      ...
      try {
        ...
        log.error(
          `MITMController: Account ${this.account?.id}, error reading certificate:`,
          e,
        );
      ...
      const requestDataTrimmed = request.certificate.data
        .trim()
        .replace(/\r/g, "");
    ...
    log.info(
      `MITMController: Account ${this.account?.id}, proxy listening on port ${this.proxyPort}`,
    );
    ...
    while (!success) {
      try {
        log.debug(
          `MITMController: Account ${this.account?.id}, making request through proxy`,
        );
        const resp = await ses.fetch(testURL, { method: "GET" });
        log.debug(
          `MITMController: Account ${this.account?.id}, got response: ${resp.status}`,
        );
        success = true;
      } catch (e) {
        log.debug(
          `MITMController: Account ${this.account?.id}, proxy not ready yet, waiting 500ms`,
          e,
        );
