...
import {
  ...
  Conversation,
  Message,
} from "../../archive-static-sites/x-archive/src/types";
...
export const selectUsers = (db: Database.Database): Record<string, User> => {
  ...
  const users: Record<string, User> = rows.reduce(
    (acc, user) => {
      acc[user.userID] = {
        userID: user.userID,
...
export const saveArchive = (
  ...
  try {
    ...
    streamWriter.write("{\n");
    streamWriter.write(
      `  "appVersion": ${JSON.stringify(xArchive.appVersion)},\n`,
    );
    streamWriter.write(`  "username": ${JSON.stringify(xArchive.username)},\n`);
    streamWriter.write(
      `  "createdAt": ${JSON.stringify(xArchive.createdAt)},\n`,
    );
